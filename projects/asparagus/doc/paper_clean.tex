\documentclass[final, 3p, times]{elsarticle} %review, twocolumn, 5p, 12pt, authoryear, twocolumn, 5p,
...
% %%%%% %%%%% %%%%% DATA AND METHODS %%%%% %%%%% %%%%% %%%%%
    ...
    photovoltaics (PV) or wind turbines, according to a substantial body of literature~\citep{Rodriguez2015}.\todo{references}
    ...
    wind power~\citep{Rodriguez2015}\todo{references}.
    Such undertakings frequently implement upper bounds on the deployment of RET, which are thought to reflect the
    available potential for RET erection~\citep{references}\todo{references}.
    ...
    \todo[inline]{refer to excluded areas as black, feasible areas as white}
    ...
    modelling~\citep{Hoeltinger2016}, by xxx~\citep{Jaeger2016} or xxx~\citep{Harper2019}\todo[inline]{add mini review paragraph}

    In contrast, the work of~\cite{Drechsler2011} % Drechsler et al. 2011: Combining spatial modelling and choice experiments ...
    \todo[inline]{mention later paper by Drechsler} \cite{Drechsler2017}
    \todo[inline]{refer to negative externalities not included in system cost}

    We improve on this work by xxx. \todo[inline]{explain what we do also in contrast to Drechsler et al.}
    ...
    \bigskip
    \todo[inline]{the following subsection must be overhauled}
    ...
    based on wind power~\citep{Rodriguez2015}. \todo[inline]{references}
    ...
    \todo[inline]{1) motivate Austrian case, 2) make clear that paper only looks at opportunity cost of wind power --
    cost of undisturbed landscapes, NOT at harm, which is assessed in various ways by many papers, though not for
    Austria}
...
% ***** medea *****
    ...
    parameters.
    \todo[inline]{perfect foresight, perfect competition, reflects energy-only market}
...
% ***** Data *****
    ...
    \todo[inline]{more details on derivation of load profiles for Austria, source for Germany, scaling}
    ...
    endogenously.
    \todo[inline]{improve description of methodology for inflow approximation}
    ...
    System cost in Austria amount to $3.4$ billon \EUR in this scenario.\todo{add co2 emissions}
    ...
    \todo[inline]{describe changes in system operation}
    ...
    As thermal generation in Austria is predominantly by fossil thermal, this induces an increase in \COO emissions due
    ...
    \begin{figure*}
        \centering
        \includegraphics[width=0.8 \textwidth]{./figures/undisturbed_base.pdf}
        \caption{Opportunity cost of wind power assuming PV overnight cost of 630 EUR/kWp}
