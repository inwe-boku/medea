...
wind_simulated = pd.read_excel('D:/git_repos/medea/data/raw/Wind_profile_4MW.xlsx')
wind_simulated.loc[:, 'time'] = pd.to_datetime(wind_simulated.loc[:, 'time'])
wind_simulated.set_index('time', inplace=True)
wind_simulated = wind_simulated.loc[
                     (pd.Timestamp(cfg.year, 1, 1, 0, 0) <= wind_simulated.index) &
                     (wind_simulated.index <= pd.Timestamp(cfg.year, 12, 31, 23,
                                                           0))] * 0.95  # 0.95 accounts for downtime and wakes
# drop index and set index of df_time instead
if len(wind_simulated) == len(dict_sets['t']):
    wind_simulated.set_index(dict_sets['t'].index, inplace=True)
else:
    raise ValueError('Mismatch of time series data and model time resolution. Is cfg.year wrong?')
wind_simulated.columns = wind_simulated.columns.str.split('-', expand=True)

FUTURE_WIND_PROFILE = df2gdx(db,
                             wind_simulated.loc[:, idx[:, :, 'profile_sim']].stack((0, 1)).reorder_levels((1, 0, 2)),
