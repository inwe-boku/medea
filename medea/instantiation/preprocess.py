...
for reg in cfg.regions:
    ...
    tec_props[('product', reg)] = 'power'
    # add column for fuel type
    tec_props[('fuel', reg)] = tec_props.index
    tec_props[('fuel', reg)] = (tec_props[('fuel', reg)]/10).apply(np.floor)*10
    tec_props[('fuel', reg)] = tec_props[('fuel', reg)].replace({y: x for x, y in fuel_set.items()})
...
tec_props.set_index('product', append=True, inplace=True)
tec_props.set_index('fuel', append=True, inplace=True)
# TODO: is dropna sensible or should all combinations be included?
