...
STORAGE_PROPERTIES = df2gdx(db, storage_clusters[df_props.index].stack().reorder_levels([1, 0, 2]).round(4),
                            'STORAGE_PROPERTIES', 'par', [r_set, tec_strg_set, prop_set])
...
PRICE_DA = df2gdx(db, ts_price.loc[:, idx['price_day_ahead', :]].stack(), 'PRICE_DA', 'par', [t_set, r_set])
PRICE_EUA = df2gdx(db, ts_price.loc[:, idx['EUA', :]].stack(), 'PRICE_EUA', 'par', [t_set, r_set])
PRICE_FUEL = df2gdx(db, ts_price.drop(['EUA', 'price_day_ahead'], axis=1).stack((0, 1)).reorder_levels(
    (0, 2, 1)).round(4), 'PRICE_FUEL', 'par', [t_set, r_set, f_set])
RESERVOIR_INFLOWS = df2gdx(db, ts_inflows.stack((0, 1)).reorder_levels((1, 0, 2)).astype('float').round(4),
                           'RESERVOIR_INFLOWS', 'par', [r_set, t_set, tec_strg_set])
